...
export default async function ParticipantTimelinePage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-slate-900 mb-1">Event Timeline</h1>
      <p className="text-slate-500 text-sm mb-6">The full schedule — general events and every team&apos;s presentation slot.</p>
      ...
      <div className="relative border-l-2 border-slate-200 ml-3">
            ...
            <div key={item.id} className="mb-5 ml-6 relative">
              <span
                className={`absolute -left-[31px] top-1 w-3 h-3 rounded-full border-2 border-white ${
                  isPast ? "bg-slate-300" : item.kind === "PRESENTATION" ? "bg-indigo-500" : "bg-emerald-500"
                }`}
              />
        ...
        {merged.length === 0 && <p className="text-sm text-slate-400 ml-6">Nothing on the schedule yet.</p>}
      </div>
    </div>
  );
}
