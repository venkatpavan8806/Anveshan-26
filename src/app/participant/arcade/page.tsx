...
export default async function ArcadePage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-slate-900 mb-1">Arcade</h1>
      <p className="text-slate-500 text-sm mb-6">Waiting between sessions? Play a quick game.</p>

      <div className="grid sm:grid-cols-2 gap-4 mb-8">
        {GAMES.map((g) => (
          <Link key={g.key} href={g.href}>
            <Card className="p-5 hover:border-indigo-300 transition h-full">
        ...
        {GAMES.map((g) => {
          ...
          return (
            <Card key={g.key} className="p-4">
              ...
              {top.length === 0 ? (
                <p className="text-xs text-slate-400">No scores yet — be the first!</p>
              ) : (
